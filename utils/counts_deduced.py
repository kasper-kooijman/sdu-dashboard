...
from .doc_search import get_requests_and_clicks_per_day_doc_search
...
def get_requests_and_clicks_per_day(clickdata: pd.DataFrame, search: MongoClient):
    ...
    non_deduced_clicks = get_requests_and_clicks_per_day_doc_search(search=search)
    clicks_per_day.extend(non_deduced_clicks)
...
def count_clicks_per_user(clickdata: pd.DataFrame, recurring_only: bool = False):
    clicks_per_user = (
        pd.DataFrame(clickdata.groupby(["user_id"]).size())
        .reset_index()
...
def get_number_of_recurring_users_in_a_week(
    start_date: datetime, clickdata: pd.DataFrame
):
    end_date = start_date + timedelta(days=7)
    weekly_interval = clickdata[
        (clickdata["date"] > start_date) & (clickdata["date"] < end_date)
    ]
    weekly_interval["datestr"] = [
        cd.strftime("%Y-%m-%d") for cd in weekly_interval["date"]
    ]
